...
def create_benchmark_chain(
    ...
    if critic_config["type"]:
        ...
        if critic_config["type"] == "lac":
            critic = create_lac_critic(
                model=model,
                temperature=model_config.get("temperature", 0.7),
                feedback_weight=critic_config.get("feedback_weight", 0.7),
                max_improvement_iterations=critic_config.get("max_iterations", 2),
            )
    ...
    for model_config in model_configs:
        for critic_config in critic_configs:
            for prompt_config in test_prompts:
                print(f"Benchmarking {model_config['name']} with {critic_config['name']} on {prompt_config['name']}...")
...
def analyze_results(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for result in results["results"]:
        ...
        analysis["critics"][critic]["validation_pass_rate"] += 1 if result["validation_passed"] else 0
        ...
        analysis["categories"][category]["validation_pass_rate"] += 1 if result["validation_passed"] else 0
        analysis["categories"][category]["improvement_rate"] += 1 if result["improvement_made"] else 0
    
    # Calculate averages
    if analysis["overall"]["successful_runs"] > 0:
        analysis["overall"]["average_time"] = total_time / analysis["overall"]["successful_runs"]
        analysis["overall"]["validation_pass_rate"] = total_validation_passes / analysis["overall"]["successful_runs"]
        analysis["overall"]["improvement_rate"] = total_improvements / analysis["overall"]["successful_runs"]
    
    for model in analysis["models"]:
        if analysis["models"][model]["successful_runs"] > 0:
            analysis["models"][model]["average_time"] /= analysis["models"][model]["successful_runs"]
            analysis["models"][model]["validation_pass_rate"] /= analysis["models"][model]["successful_runs"]
            analysis["models"][model]["improvement_rate"] /= analysis["models"][model]["successful_runs"]
    
    for critic in analysis["critics"]:
        if analysis["critics"][critic]["successful_runs"] > 0:
            analysis["critics"][critic]["average_time"] /= analysis["critics"][critic]["successful_runs"]
            analysis["critics"][critic]["validation_pass_rate"] /= analysis["critics"][critic]["successful_runs"]
            analysis["critics"][critic]["improvement_rate"] /= analysis["critics"][critic]["successful_runs"]
    
    for category in analysis["categories"]:
        if analysis["categories"][category]["successful_runs"] > 0:
            analysis["categories"][category]["average_time"] /= analysis["categories"][category]["successful_runs"]
            analysis["categories"][category]["validation_pass_rate"] /= analysis["categories"][category]["successful_runs"]
            analysis["categories"][category]["improvement_rate"] /= analysis["categories"][category]["successful_runs"]
...
def main():
    ...
    # Run the benchmark
    print(f"Running benchmark with {len(model_configs)} models, {len(critic_configs)} critics, and {len(test_prompts)} prompts...")

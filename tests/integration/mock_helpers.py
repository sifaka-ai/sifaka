...
def mock_llm_calls(provider: str = "openai") -> Generator[MagicMock, None, None]:
    ...
    if provider == "openai":
        ...
        patches.extend([
            patch("sifaka.llms.google.genai.GenerativeModel", return_value=mock_llm),
        ])
...
def mock_improve_function():
    ...
    def mock_improve_sync(text: str, **kwargs) -> Any:
        ...
        critic = kwargs.get("critics", ["reflexion"])[0] if kwargs.get("critics") else "reflexion"
    ...
    with patch("sifaka.improve_sync", side_effect=mock_improve_sync), \

...
class PerformanceMeasurer:
    ...
    def measure_batch_operations(
        self, operation_func, batch_size: int, *args, **kwargs
    ) -> PerformanceMetrics:
...
class TestBasicPerformance:
    ...
    def test_simple_chain_performance(self):
        ...
        ), f"Simple chain uses too much memory: {metrics.memory_usage_mb:.1f}MB"

        logger.info(
            f"Simple chain performance: {metrics.execution_time:.3f}s, {metrics.memory_usage_mb:.1f}MB"
        )
    ...
    def test_chain_with_validators_performance(self):
        ...
        ), f"Validated chain too slow: {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_usage_mb < 75
        ), f"Validated chain uses too much memory: {metrics.memory_usage_mb:.1f}MB"

        logger.info(
            f"Validated chain performance: {metrics.execution_time:.3f}s, {metrics.memory_usage_mb:.1f}MB"
        )
    ...
    def test_chain_with_critics_performance(self):
        ...
        ), f"Criticized chain too slow: {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_usage_mb < 100
        ), f"Criticized chain uses too much memory: {metrics.memory_usage_mb:.1f}MB"

        logger.info(
            f"Criticized chain performance: {metrics.execution_time:.3f}s, {metrics.memory_usage_mb:.1f}MB"
        )
...
class TestThroughputPerformance:
    ...
    def test_sequential_chain_throughput(self):
        ...
        ), f"Sequential throughput too low: {metrics.throughput:.2f} ops/sec"
        assert (
            metrics.execution_time < 10.0
        ), f"Batch execution too slow: {metrics.execution_time:.3f}s"

        logger.info(
            f"Sequential throughput: {metrics.throughput:.2f} ops/sec ({batch_size} operations in {metrics.execution_time:.3f}s)"
        )
    ...
    def test_concurrent_chain_performance(self):
        ...
        logger.info(
            f"Concurrent throughput: {concurrent_throughput:.2f} ops/sec ({len(results)} operations in {concurrent_time:.3f}s)"
        )
    ...
    def test_memory_efficiency_batch_processing(self):
        ...
        assert (
            memory_increase < 100
        ), f"Memory increase too high: {memory_increase:.1f}MB for 20 chains"
...
class TestScalabilityPerformance:
    ...
    def test_validator_scalability(self):
        ...
        for count in validator_counts:
            ...
            assert (
                metrics.execution_time < 5.0
            ), f"Chain with {count} validators too slow: {metrics.execution_time:.3f}s"
        ...
        assert (
            scaling_factor < 5.0
        ), f"Validator scaling too poor: {scaling_factor:.2f}x increase from {validator_counts[0]} to {validator_counts[-1]} validators"

        logger.info(
            f"Validator scalability: {execution_times} (scaling factor: {scaling_factor:.2f}x)"
        )
    ...
    def test_critic_scalability(self):
        ...
        for count in critic_counts:
            ...
            def run_chain_with_critics():
                ...
                chain = Chain(
                    model=model, prompt="Write about critic scalability.", always_apply_critics=True
                )
            ...
            assert (
                metrics.execution_time < 10.0
            ), f"Chain with {count} critics too slow: {metrics.execution_time:.3f}s"
...
class TestRegressionBenchmarks:
    ...
    def test_baseline_chain_benchmark(self):
        ...
        logger.info(
            f"Baseline benchmark: {avg_time:.3f}s average ({min(times):.3f}s - {max(times):.3f}s)"
        )
    ...
    def test_complex_chain_benchmark(self):
        ...
        assert (
            metrics.execution_time < 8.0
        ), f"Complex chain performance regression: {metrics.execution_time:.3f}s"
        assert (
            metrics.memory_usage_mb < 150
        ), f"Complex chain memory regression: {metrics.memory_usage_mb:.1f}MB"

        logger.info(
            f"Complex benchmark: {metrics.execution_time:.3f}s, {metrics.memory_usage_mb:.1f}MB"
        )
    ...
    def test_concurrent_benchmark(self):
        ...
        assert (
            execution_time < 5.0
        ), f"Concurrent performance regression: {execution_time:.3f}s for 10 operations"

...
class TestContentGenerationScenarios:
    ...
    def test_blog_post_generation(self):
        ...
        assert_critic_feedback(result, expected_count=2)
        assert_chain_execution_success(result)
        assert_performance_within_bounds(execution_time, max_time=10.0, operation_name="blog post generation")
    ...
    def test_technical_documentation(self):
        ...
        chain.validate_with(RegexValidator(required_patterns=[r"API", r"GET|POST|PUT|DELETE", r"Response"]))
        chain.validate_with(ContentValidator(prohibited=["TODO", "FIXME"], name="Completeness Check"))
        ...
        assert_critic_feedback(result, expected_count=1)
        assert_chain_execution_success(result)
    ...
    def test_creative_writing_scenario(self):
        ...
        chain.validate_with(RegexValidator(required_patterns=[r"technology|tech", r"magic|fantasy"]))
...
class TestMultiStepWorkflows:
    ...
    def test_iterative_improvement_workflow(self):
        ...
        chain.improve_with(ReflexionCritic(model=critic_model))
        chain.improve_with(ConstitutionalCritic(
            model=critic_model,
            principles=["Be accurate", "Be persuasive", "Be clear"]
        ))
...
class TestErrorRecoveryScenarios:
    ...
    def test_critic_failure_graceful_handling(self):
        ...
        failing_critic = MockModelFactory.create_failing(error_message="Critic temporarily unavailable")
        ...
        assert_chain_execution_success(result)
        # Should have feedback from the working critic
    ...
    def test_storage_failure_handling(self):
        ...
        chain = Chain(
            model=model,
            prompt="Write about data persistence.",
...
class TestPerformanceScenarios:
    ...
    def test_high_throughput_scenario(self):
        ...
        for i in range(5):
            chain = Chain(
                model=model,
                prompt=f"Write a brief summary about topic {i}."
            )
        ...
        assert avg_time_per_request < 2.0, f"Average time per request too high: {avg_time_per_request:.2f}s"
    ...
    def test_complex_validation_performance(self):
        ...
        chain = Chain(model=model, prompt="Write about AI comprehensively.")
        
        # Multiple validators
        chain.validate_with(LengthValidator(min_length=100, max_length=1000))
        chain.validate_with(RegexValidator(required_patterns=[r"artificial", r"learning", r"neural"]))
        ...
        assert_validation_results(result, expected_count=3)
        assert_critic_feedback(result, expected_count=2)
        assert_performance_within_bounds(execution_time, max_time=15.0, operation_name="complex validation")
...
class TestRealWorldUseCases:
    ...
    def test_customer_support_response(self):
        ...
        chain.validate_with(ContentValidator(prohibited=["sorry", "unfortunately"], name="Positive Tone"))
    ...
    def test_educational_content_creation(self):
        ...
        chain.validate_with(RegexValidator(required_patterns=[r"Python", r"programming", r"example"]))
        ...
        chain.improve_with(ConstitutionalCritic(
            model=critic_model,
            principles=["Be clear", "Be educational", "Include examples"]
        ))

#!/usr/bin/env python3
"""
Comprehensive test suite for Sifaka unified storage implementation.

This module tests all aspects of the unified storage layer including:
...
from sifaka.storage import CachedThoughtStorage, ThoughtQuery
from sifaka.storage.base import InMemoryStorage, CachedStorage


def create_test_storage() -> CachedThoughtStorage:
    """Create a memory-only storage for testing."""
    memory = InMemoryStorage(max_size=1000)
    storage_backend = CachedStorage(memory=memory, cache=None, persistence=None)
    return CachedThoughtStorage(storage_backend)


def test_basic_persistence() -> None:
    ...
    critic_feedback = CriticFeedback(
        ...
        violations=["Could use more vivid descriptions"],
    ...
    assert retrieved_thought.pre_generation_context is not None, "Context should not be None"
    assert len(retrieved_thought.pre_generation_context) == 2, "Context not preserved"
    assert (
        retrieved_thought.pre_generation_context[0].text
        == "Robots are mechanical beings capable of learning."
    )
    print("✓ Context preserved correctly")

    # Check validation results preservation
    assert retrieved_thought.validation_results is not None, "Validation results should not be None"
    assert (
        "length_validator" in retrieved_thought.validation_results
    ), "Validation results not preserved"
    ...
    assert retrieved_thought.critic_feedback is not None, "Critic feedback should not be None"
...
def test_querying() -> None:
    ...
    assert ai_results.thoughts[0].text is not None and "intelligence" in ai_results.thoughts[0].text
    ...
    assert (
        chain_results.total_count == 2
    ), f"Expected 2 chain thoughts, got {chain_results.total_count}"
    ...
    assert (
        validation_results.total_count == 1
    ), f"Expected 1 validated thought, got {validation_results.total_count}"
...
def test_health_check() -> None:
    ...
    print(f"  - Storage type: Memory-based")
...
def main() -> int:
    """Run all tests."""
    print("🧪 Testing Sifaka Unified Storage Implementation")
    print("=" * 50)
    ...
    try:
        ...
        print("\n" + "=" * 50)
        print("🎉 All tests passed! Unified storage is working correctly.")

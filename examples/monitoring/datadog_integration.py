...
class DataDogMiddleware(BaseMiddleware):
    ...
    async def pre_improve(self, text: str, **kwargs) -> Dict[str, Any]:
        ...
        self.statsd.gauge('active_improvements', 1, tags=[
            f"critic:{context['critic']}"
        ])
        
        # Track input text length
        self.statsd.histogram('text.length.input', len(text), tags=[
            f"critic:{context['critic']}"
        ])
    ...
    async def post_improve(
        ...
        if error:
            # Track error
            self.statsd.increment('errors', tags=[
                f"critic:{context['critic']}",
            ...
            # Track iterations
            self.statsd.histogram('improvement.iterations', result.iterations, tags=tags)
            
            # Track tokens
            self.statsd.increment('tokens.used', result.total_tokens, tags=[
                f"model:{context['model']}",
                'operation:improvement'
            ])
            
            # Track cost
            self.statsd.histogram('improvement.cost', result.total_cost, tags=[
                f"model:{context['model']}"
            ])
            
            # Track output text length
            self.statsd.histogram('text.length.output', len(result.final_text), tags=[
                f"critic:{context['critic']}"
            ])
    ...
    async def on_critique(self, critique: str, iteration: int, **kwargs):
        ...
        with tracer.start_span(
            ...
            span.set_tags({
                'iteration': iteration,
                'critique.length': len(critique)
            })
            
            # Track critique generation
            self.statsd.increment('critiques.generated', tags=[
    ...
    async def on_generation(self, text: str, iteration: int, **kwargs):
        ...
        with tracer.start_span(
            ...
            # Track generation
            self.statsd.increment('generations.created', tags=[
                f"iteration:{iteration}",
                f"critic:{kwargs.get('critic', 'unknown')}"
            ])
    ...
    async def on_validation(self, validation_result: Any, validator: str, **kwargs):
        ...
        self.statsd.increment('validations', tags=[
...
class SifakaBusinessMetrics:
    ...
    def track_improvement_quality(self, original: str, improved: str, critic: str):
        ...
        self.statsd.histogram('improvement.quality.ratio', ratio, tags=[
            f"critic:{critic}"
        ])
        
        # Track if text was shortened or lengthened
        if ratio > 1.2:
            self.statsd.increment('improvement.quality.expanded', tags=[f"critic:{critic}"])
        elif ratio < 0.8:
            self.statsd.increment('improvement.quality.condensed', tags=[f"critic:{critic}"])
        else:
            self.statsd.increment('improvement.quality.maintained', tags=[f"critic:{critic}"])
    
    def track_user_satisfaction(self, user_id: str, satisfied: bool, critic: str):
        """Track user satisfaction metrics."""
        self.statsd.increment('user.satisfaction', tags=[
    ...
    def track_api_usage(self, user_id: str, endpoint: str, response_time: float):
        ...
        self.statsd.histogram('api.response_time', response_time * 1000, tags=[
            f"endpoint:{endpoint}",
            f"user_id:{user_id}"
        ])
        
        self.statsd.increment('api.requests', tags=[
            f"endpoint:{endpoint}",
            f"user_id:{user_id}"
        ])
...
if __name__ == "__main__":
    ...
    async def process_batch():
        ...
        with tracer.trace("batch.processing", service="sifaka-example") as batch_span:
            ...
            dd_middleware.statsd.gauge('batch.success_rate', 
                                     success_count / len(texts) * 100)
    ...
    print("   sum:sifaka.improvements{status:success}.as_count() / sum:sifaka.improvements{*}.as_count() * 100")

...
class PerformanceMonitoringMiddleware(BaseMiddleware):
    ...
    def _log_metrics(self, metrics: PerformanceMetrics, result: SifakaResult, error: Optional[Exception]):
...
class PerformanceBenchmark:
    ...
    async def benchmark_critics(self, text: str, iterations: int = 5) -> Dict[str, Any]:
        ...
        for critic in critics:
            ...
            if critic_metrics:
                results[critic] = {
                    'avg_duration': sum(m.duration_seconds for m in critic_metrics) / len(critic_metrics),
                    'avg_memory': sum(m.memory_used_mb for m in critic_metrics) / len(critic_metrics),
                    'avg_tokens_per_sec': sum(m.tokens_per_second for m in critic_metrics) / len(critic_metrics)
                }
        
        return results
    
    def benchmark_text_sizes(self, sizes: List[int] = [100, 500, 1000, 5000]) -> Dict[int, Any]:
...
if __name__ == "__main__":
    ...
    for size, metrics in size_results.items():
        if 'error' not in metrics:
            print(f"  {size} chars: {metrics['duration']:.2f}s, "
    ...
    if 'duration' in summary:
        print(f"Duration - Min: {summary['duration']['min']:.2f}s, "

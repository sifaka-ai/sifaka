...
class PrometheusMiddleware(BaseMiddleware):
    ...
    async def post_improve(
        ...
        if error:
            ...
            ).add(result.total_tokens)
            
            # Record cost
            self.cost_histogram.labels(model=model).observe(result.total_cost)
            
            # Record output text length
            self.text_length_summary.labels(type='output').observe(len(result.final_text))
    ...
    def collect(self):
        ...
        if hasattr(self.app, 'cache'):
            ...
            yield CounterMetricFamily(
                'sifaka_cache_hits_total',
                'Total cache hits',
                value=self.app.cache.hits
            )
...
if __name__ == "__main__":
    ...
    print("sifaka_improvement_duration_seconds_sum / sifaka_improvement_duration_seconds_count")
    print("\n# Error rate:")
    ...
    print("histogram_quantile(0.95, rate(sifaka_improvement_duration_seconds_bucket[5m]))")

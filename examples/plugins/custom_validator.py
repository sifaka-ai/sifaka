...
class SEOValidator(BaseValidator):
    ...
    async def validate(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> ValidationResult:
        ...
        if word_count < self.ideal_length[0]:
            issues.append(f"Content too short: {word_count} words (minimum: {self.ideal_length[0]})")
            suggestions.append("Expand content with more detailed information")
        elif word_count > self.ideal_length[1]:
            issues.append(f"Content too long: {word_count} words (maximum: {self.ideal_length[1]})")
        ...
        for keyword in self.target_keywords:
            ...
            if density < self.min_keyword_density:
                ...
                suggestions.append(f"Reduce repetition of '{keyword}' to avoid keyword stuffing")
        ...
        if self.require_headers:
            ...
            if len(h2_matches) < 2:
                suggestions.append("Consider adding more subheadings (H2) for better structure")
        ...
        return ValidationResult(
            ...
            metadata={
                ...
                "header_count": len(h1_matches) + len(h2_matches) if self.require_headers else 0
            }
        )
    ...
    async def validate(self, text: str, metadata: Optional[Dict[str, Any]] = None) -> ValidationResult:
        ...
        for link in links:
            ...
            if link_text.lower() in ['click here', 'here', 'read more', 'link']:
                ...
                suggestions.append("Use descriptive link text that explains the destination")
        ...
        if headings:
            ...
            for i in range(1, len(levels)):
                if levels[i] - levels[i-1] > 1:
                    ...
                    suggestions.append("Maintain proper heading hierarchy (don't skip levels)")

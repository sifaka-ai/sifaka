...
async def main() -> None:
    ...
    if os.getenv("GOOGLE_API_KEY"):
        ...
        result = await improve(
            ...
            config=Config(
                ...
                critic=CriticConfig(critics=[CriticType.REFLEXION]),
            ),
            storage=FileStorage(),
        )
    ...
    # Option 2: Anthropic Claude (excellent self-reflection)
        ...
        result = await improve(
            ...
            config=Config(
                llm=LLMConfig(
                    model="gpt-4o-mini", critic_model="gpt-4o-mini", temperature=0.6
                ),
                critic=CriticConfig(critics=[CriticType.REFLEXION]),
            ),
            storage=FileStorage(),
...
async def reflexion_comparison():
    ...
    if os.getenv("GOOGLE_API_KEY"):
        ...
        result = await improve(
            ...
            max_iterations=2,
            config=Config(
                llm=LLMConfig(model="gpt-4o-mini", critic_model="gpt-4o-mini"),
                critic=CriticConfig(critics=[CriticType.REFLEXION]),
            ),
        )

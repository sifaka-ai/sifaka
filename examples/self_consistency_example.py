...
async def main():
    ...
    try:
        ...
        result = await improve(text, critics=["self_consistency"], max_iterations=3)
        ...
        consistency_critiques = [
            c for c in result.critiques if c.critic == "self_consistency"
        ]
        ...
        if consistency_critiques:
            ...
            for critique in consistency_critiques:
                ...
                iteration = len(
                    [
                        c
                        for c in consistency_critiques
                        if c.timestamp <= critique.timestamp
                    ]
                )
            ...
            for iteration, critiques in by_iteration.items():
                ...
                print(
                    f"  Consensus: {'Strong' if avg_confidence > 0.7 else 'Moderate' if avg_confidence > 0.5 else 'Weak'}"
                )

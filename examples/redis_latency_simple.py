...
async def test_direct_redis_latency(num_operations: int = 20) -> Dict[str, Any]:
    ...
    storage = RedisStorage(
        host="localhost", port=6379, prefix="sifaka:latency:direct:", ttl=3600
    )
    ...
    for i in range(num_operations):
        ...
        print(
            f"   Op {i+1:2d}: Save={save_time:5.1f}ms, Load={load_time:5.1f}ms | Total={total_time:6.1f}ms"
        )
    ...
    print("   Operation  │   Avg   │   Min   │   Max   │ Median  │   P95   ")
    print("   ───────────┼─────────┼─────────┼─────────┼─────────┼─────────")
    print(
        f"   Save       │ {save_stats['avg']:6.1f}ms│ {save_stats['min']:6.1f}ms│ {save_stats['max']:6.1f}ms│ {save_stats['median']:6.1f}ms│ {save_stats['p95']:6.1f}ms"
    )
    print(
        f"   Load       │ {load_stats['avg']:6.1f}ms│ {load_stats['min']:6.1f}ms│ {load_stats['max']:6.1f}ms│ {load_stats['median']:6.1f}ms│ {load_stats['p95']:6.1f}ms"
    )
...
def compare_latencies(direct_results: Dict[str, Any], mcp_results: Dict[str, Any]):
    ...
    print("   Operation  │  Direct  │   MCP    │ Overhead │ Slowdown ")
    print("   ───────────┼──────────┼──────────┼──────────┼──────────")
    ...
    for op in operations:
        ...
        print(
            f"   {op.capitalize():<10} │ {direct_avg:7.1f}ms│ {mcp_avg:7.1f}ms│ +{overhead:6.1f}ms│ {slowdown:6.1f}x"
        )
    ...
    for op in operations:
        ...
        print(
            f"   {op.capitalize():<10} │ {direct_p95:7.1f}ms│ {mcp_p95:7.1f}ms│ +{overhead:6.1f}ms│ {slowdown:6.1f}x"
        )

    # Key insights
    print("\n💡 Key Insights:")
    avg_slowdown = sum(
        (mcp_results[f"{op}_stats"]["avg"] / direct_results[f"{op}_stats"]["avg"])
        for op in operations
    ) / len(operations)
    print(f"   • MCP Redis is {avg_slowdown:.1f}x slower on average")
    print(f"   • Total overhead per operation cycle: +{total_overhead:.1f}ms")
    print("   • MCP overhead comes from process communication and JSON-RPC protocol")
    print("   • Direct Redis has sub-millisecond latency for most operations")
    print("   • MCP Redis suitable for non-latency-critical applications")

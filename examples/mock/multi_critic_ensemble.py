...
def main():
    ...
    model = MockModel(
        name="Educational Content Model",
        responses=[
    ...
    length_validator = LengthValidator(
        min_length=100,
        max_length=1000,
        name="Educational Content Length Validator"
    )
    ...
    if result.critic_feedback:
        ...
        for i, feedback in enumerate(result.critic_feedback, 1):
            ...
            if hasattr(feedback, 'principles_evaluated'):
                print(f"     Constitutional Principles: {len(feedback.principles_evaluated)} evaluated")
    ...
    for i, historical_thought in enumerate(result.history, 1):
        print(f"\n  Iteration {i} ({len(historical_thought.text)} characters):")
        print(f"    Preview: {historical_thought.text[:120]}...")

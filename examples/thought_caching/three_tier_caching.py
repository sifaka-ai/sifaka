...
def create_three_tier_storage():
    ...
    redis_storage = RedisStorage(
        host="localhost",
        port=6379,
        db=4,  # Use different DB for thought caching
        key_prefix="sifaka:thoughts:",
...
def demonstrate_tier_access(cached_storage, memory_storage, redis_storage, milvus_storage, thought):
    ...
    try:
        ...
        print(f"  Milvus Tier: {'✓ Found' if milvus_result else '✗ Not found'} ({milvus_time:.2f}ms)")
    except Exception as e:
...
def main():
    ...
    critic = SelfRefineCritic(
        model=model,
        max_refinements=2,
        name="Distributed Systems Self-Refine Critic"
    )
    ...
    length_validator = LengthValidator(
        min_length=150,
        max_length=800,
        name="Technical Content Length Validator"
    )

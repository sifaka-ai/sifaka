...
async def composable_validators_example():
    ...
    technical_validator = Validator.length(200, 1000) & Validator.contains(
        ["algorithm", "performance", "complexity"], mode="any"
    )

    # Either email OR phone number
    contact_validator = Validator.matches(
        r"\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z|a-z]{2,}\b", "email"
...
async def middleware_with_monitoring():
    ...
    for text in texts:
        result = await improve(
            text, critics=["reflexion"], max_iterations=1, middleware=pipeline
        )
...
async def custom_validator_example():
    ...
    seo_validator = (
        ...
        .custom(
            ...
            check=lambda text: 0.01
            <= text.lower().count("keyword") / len(text.split())
            <= 0.03,

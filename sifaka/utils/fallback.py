...
class FallbackChain:
    ...
    def add_primary(self, func: Callable[..., Any], name: Optional[str] = None) -> None:
    ...
    def add_fallback(
        self, func: Callable[..., Any], priority: int = 1, name: Optional[str] = None
    ) -> None:
        ...
        logger.debug(
            f"Added fallback service '{name}' (priority {priority}) to {self.service_name}"
        )
    ...
    def _execute_option(self, option: FallbackOption, *args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            self.stats.fallback_usage[option.name] = (
                self.stats.fallback_usage.get(option.name, 0) + 1
            )
    ...
    def execute(self, *args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                if option.name == "primary":
                    ...
                    fallback_successes = sum(
                        count
                        for name, count in self.stats.fallback_usage.items()
                        if name != "primary"
                    )
        ...
        raise FallbackError(
            self.service_name, attempted_options, last_exception or Exception("Unknown error")
        )
    ...
    def health_check(self) -> Dict[str, bool]:
        ...
        for option in all_options:
            ...
            if (
                option.last_health_check is None
                or current_time - option.last_health_check >= self.config.health_check_interval
            ):
...
def create_model_fallback_chain(
    primary_model: Any, fallback_models: List[Any], config: Optional[FallbackConfig] = None
) -> FallbackChain:
...
def create_retriever_fallback_chain(
    primary_retriever: Any, fallback_retrievers: List[Any], config: Optional[FallbackConfig] = None
) -> FallbackChain:

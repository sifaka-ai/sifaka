...
class ComposableValidator(BaseValidator):
    ...
    def __invert__(self) -> 'ComposableValidator':
        ...
        for rule in self.rules:
            ...
            def make_inverted_check(orig: Callable[[str], bool]) -> Callable[[str], bool]:
                return lambda text: not orig(text)
            
            def make_inverted_score(orig: Callable[[str], float]) -> Callable[[str], float]:
                return lambda text: 1.0 - orig(text)
            
            def make_inverted_detail(orig: Callable[[str], str]) -> Callable[[str], str]:
...
class ValidatorBuilder:
    ...
    def length(self, min_length: int = 0, max_length: int = 999999) -> 'ValidatorBuilder':
    ...
    def custom(
        ...
        rule = ValidationRule(
            ...
            detail_func=detail or (lambda text: f"{name}: {'passed' if check(text) else 'failed'}")
        )
        self.rules.append(rule)
        return self
    
    def sentences(self, min_sentences: int = 1, max_sentences: int = 999999) -> 'ValidatorBuilder':
        ...
        rule = ValidationRule(
            ...
            score_func=lambda text: min(1.0, count_sentences(text) / max(min_sentences, 5)),
...
class Validator:
    ...
    @staticmethod
    def sentences(min_sentences: int = 1, max_sentences: int = 999999) -> ComposableValidator:
        """Create a sentence count validator."""
        return ValidatorBuilder("sentences").sentences(min_sentences, max_sentences).build()

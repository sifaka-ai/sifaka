...
class LengthValidatorBase(BaseValidator):
    ...
    def validate(self, thought: Thought) -> ValidationResult:
        """Validate text with length-specific handling for empty text.
        ...
        with validation_context(
            ...
            # For length validators, we need to handle empty text specially
            # because empty text might be valid if min_length=0
            if thought.text is None:
                return self.create_empty_text_result(self.name)

            # Allow empty string to be processed by length validation
            try:
                return self._validate_content(thought)
            except Exception as e:
                return self.create_error_result(e, self.name)
    ...
    def _validate_content(self, thought: Thought) -> ValidationResult:
        ...
        if passed:
            ...
            # Create message that includes the expected keywords for tests
            if self.min_length is not None and length < self.min_length:
                message = (
                    f"Text is too short: {length} {self.unit} (minimum required: {self.min_length})"
                )
            elif self.max_length is not None and length > self.max_length:
                message = (
                    f"Text is too long: {length} {self.unit} (maximum allowed: {self.max_length})"
                )
            else:
...
class RegexValidatorBase(BaseValidator):
    ...
    def __init__(
        self, patterns: Dict[str, str], mode: str = "require_all", name: Optional[str] = None
    ):
    ...
    def _validate_content(self, thought: Thought) -> ValidationResult:
        ...
        if self.mode == "require_all":
            ...
            if missing_patterns:
                ...
                suggestions.extend(
                    [f"Include content matching pattern '{name}'" for name in missing_patterns]
                )
            ...
            if not any(matches.values()):
                ...
                suggestions.append(
                    f"Include content matching at least one of: {', '.join(pattern_names)}"
                )
            ...
            if found_patterns:
                ...
                suggestions.extend(
                    [f"Remove content matching pattern '{name}'" for name in found_patterns]
                )
    ...
    def _validate_content(self, thought: Thought) -> ValidationResult:
        ...
        try:
            ...
            if self.valid_labels and predicted_label not in self.valid_labels:
                passed = False
                issues.append(
                    f"Text classified as '{predicted_label}' which is not in valid labels: {self.valid_labels}"
                )
                suggestions.append(
                    f"Modify text to align with valid categories: {self.valid_labels}"
                )

            if self.invalid_labels and predicted_label in self.invalid_labels:
                passed = False
                issues.append(
                    f"Text classified as '{predicted_label}' which is in invalid labels: {self.invalid_labels}"
                )
                suggestions.append(
                    f"Modify text to avoid invalid categories: {self.invalid_labels}"
                )

    ...
    def __init__(
        ...
        if min_length is not None and min_length < 0:
            raise ValidationError(
                ...
                suggestions=["Use a non-negative minimum length", "Set min_length to None for no minimum"]
            )
        ...
        if max_length is not None and max_length < 0:
            raise ValidationError(
                ...
                suggestions=["Use a non-negative maximum length", "Set max_length to None for no maximum"]
            )
    ...
    async def validate_async(self, thought: SifakaThought) -> ValidationResult:
        ...
        with self.time_operation("length_validation") as timer:
            ...
            if self.min_length is not None and length < self.min_length:
                ...
                suggestions.append(f"Add at least {deficit} more {self.unit} to meet minimum length")
            ...
            if self.max_length is not None and length > self.max_length:
                ...
                suggestions.append(f"Remove at least {excess} {self.unit} to meet maximum length")
            ...
            if passed:
                message = self.format_length_message(length, self.unit, self.min_length, self.max_length)
...
def min_length_validator(min_length: int, unit: str = "characters", strict: bool = True) -> LengthValidator:
...
def max_length_validator(max_length: int, unit: str = "characters", strict: bool = True) -> LengthValidator:

...
class MilvusPersistence(SifakaBasePersistence):
        ...
        if self.connection is None:
            try:
                ...
                schema = CollectionSchema(
                    fields=fields,
                    description="Sifaka thoughts with vector embeddings"
                )
                ...
                raise ImportError(
                    "pymilvus is required for Milvus persistence. "
    ...
    async def _retrieve_thought_data(self, thought_id: str) -> Optional[str]:
        ...
        results = self.collection.query(
            expr=expr,
            output_fields=["id", "prompt", "final_text", "conversation_id", "techniques", "created_at", "metadata"]
        )

        ...
        try:
            ...
            async with self.mcp_server as mcp_client:
                ...
                result = await mcp_client.call_tool(
                    "json_set", arguments={"name": key, "path": "$", "value": json_data}
                )

                # Set TTL if configured
                if self.ttl_seconds:
                    await mcp_client.call_tool(
                        "expire", arguments={"name": key, "seconds": self.ttl_seconds}
                    )
    ...
    async def _retrieve_raw(self, key: str) -> Optional[str]:
        ...
        try:
            async with self.mcp_server as mcp_client:
                ...
                result = await mcp_client.call_tool(
                    "json_get", arguments={"name": key, "path": "$"}
                )
    ...
    async def _delete_raw(self, key: str) -> bool:
        ...
        try:
            async with self.mcp_server as mcp_client:
                # Use json_del tool
                result = await mcp_client.call_tool(
                    "json_del", arguments={"name": key, "path": "$"}
                )
    ...
    async def _list_keys(self, pattern: str) -> List[str]:
        ...
        try:
            async with self.mcp_server as mcp_client:
                ...
                result = await mcp_client.call_tool("keys", arguments={"pattern": pattern})

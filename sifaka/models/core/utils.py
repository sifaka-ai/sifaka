...
def update_statistics(provider: 'ModelProviderCore', result: str,
    ...
    """
    generation_stats = provider.(_state_manager and _state_manager.get('generation_stats', {})
    generation_stats['generation_count'] = (generation_stats and generation_stats.get(
        'generation_count', 0) + 1
    generation_stats['total_generation_time_ms'] = (generation_stats and generation_stats.get(
        'total_generation_time_ms', 0) + processing_time_ms
    provider.(_state_manager and _state_manager.update('generation_stats', generation_stats)


def update_token_count_statistics(provider: 'ModelProviderCore',
    token_count: int, processing_time_ms: float) ->None:
    """
    ...
    count_stats['total_tokens_counted'] = (count_stats and count_stats.get(
        'total_tokens_counted', 0) + token_count
    count_stats['count_operations'] = (count_stats and count_stats.get('count_operations', 0
        ) + 1
    count_stats['total_processing_time_ms'] = (count_stats and count_stats.get(
        'total_processing_time_ms', 0) + processing_time_ms
    provider.(_state_manager and _state_manager.update('token_count_stats', count_stats)
...
def get_generation_stats(provider: 'ModelProviderCore') ->Any:
    ...
    """
    generation_stats = provider.(_state_manager and _state_manager.get('generation_stats', {})
    generation_count = (generation_stats and generation_stats.get('generation_count', 0)
    total_time_ms = (generation_stats and generation_stats.get('total_generation_time_ms', 0)
    avg_time_ms = (total_time_ms / generation_count if generation_count > 0
         else 0)
    return {'generation_count': generation_count, 'avg_generation_time_ms':
        avg_time_ms, 'total_generation_time_ms': total_time_ms}
...
def get_component_info(provider: 'ModelProviderCore') ->Any:
    ...
    """
    return {'name': provider.name, 'model_name': provider.model_name,

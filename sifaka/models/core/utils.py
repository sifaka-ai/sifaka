...
def update_statistics(provider: 'ModelProviderCore', result: str, processing_time_ms: float) -> None:
    """
...
def get_generation_stats(provider: 'ModelProviderCore') -> Dict[str, Any]:
    """
    ...
    """
    generation_stats = provider._state_manager.get("generation_stats", {})
    generation_count = generation_stats.get("generation_count", 0)
    total_time_ms = generation_stats.get("total_generation_time_ms", 0)
...
def get_component_info(provider: 'ModelProviderCore') -> Dict[str, Any]:
    """

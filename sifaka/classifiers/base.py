    ...
    def batch_classify(self, texts: List[str]) -> List[ClassificationResult]:
        ...
        for text in texts:
            try:
                ...
                results.append(
                    ClassificationResult(label="error", confidence=0.0, metadata={"error": str(e)})
                )
        return results
...
class CachedTextClassifier(TextClassifier):
    """Text classifier with LRU caching support.

    This class extends TextClassifier with efficient LRU caching of classification results.
    It uses Python's functools.lru_cache for optimal performance and memory management.

    The caching is based on a hash of the input text, which allows for:
    ...
    Attributes:
        cache_size: Maximum number of cached results (default: 128)
        name: The name of the classifier
        description: A description of what the classifier does
    """

    def __init__(
        self, name: str = "CachedTextClassifier", description: str = "", cache_size: int = 128
    ):
        ...
        Args:
            name: The name of the classifier
            description: A description of what the classifier does
            cache_size: Maximum number of results to cache (default: 128)
        """
        super().__init__(name, description)
        self.cache_size = cache_size

        # Create a cached version of the _classify_uncached method
        self._cached_classify = lru_cache(maxsize=cache_size)(self._classify_uncached)

    def classify(self, text: str) -> ClassificationResult:
        """Classify text with LRU caching.

        This method first checks the cache for the text. If found, returns the cached result.
        Otherwise, calls the underlying classification method and caches the result.
        ...
        if not text or not text.strip():
            # Don't cache empty/whitespace results
            return self._classify_uncached(text)

        # Use the cached method
        return self._cached_classify(text)

    @abstractmethod
    def _classify_uncached(self, text: str) -> ClassificationResult:
        """Perform the actual classification without caching.

        This method must be implemented by subclasses to provide the core
        classification logic. It will be automatically wrapped with LRU caching.
    ...
    def get_cache_info(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with cache hit/miss statistics and current size
        """

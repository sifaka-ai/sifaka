...
class RecoveryManager:
    ...
    def analyze_failure(
        self, 
        failed_checkpoint: ChainCheckpoint,
        error: Exception
    ) -> List[RecoveryAction]:
        ...
        """
        logger.info(f"Analyzing failure at step '{failed_checkpoint.current_step}' "
    ...
    def _find_similar_checkpoints(
        ...
        try:
            similar_items = self.checkpoint_storage.search_similar(query, limit=limit)
            similar_checkpoints = [
                item for item in similar_items 
                if isinstance(item, ChainCheckpoint) and item.checkpoint_id != checkpoint.checkpoint_id
            ]
    ...
    def _analyze_error_patterns(
        self, 
        error: Exception, 
        similar_checkpoints: List[ChainCheckpoint]
    ) -> Dict[str, Any]:
    ...
    def _estimate_retry_success_rate(self, similar_checkpoints: List[ChainCheckpoint]) -> float:
        ...
        successful_retries = sum(
            1 for cp in similar_checkpoints 
            if cp.metadata.get("retry_count", 0) > 0 and cp.current_step == "complete"
        )
        
        total_retries = sum(
            1 for cp in similar_checkpoints 
            if cp.metadata.get("retry_count", 0) > 0
        )
    ...
    def _estimate_restart_success_rate(self, similar_checkpoints: List[ChainCheckpoint]) -> float:
        ...
        total_restarts = sum(
            1 for cp in similar_checkpoints 
            if cp.metadata.get("restart_count", 0) > 0
        )

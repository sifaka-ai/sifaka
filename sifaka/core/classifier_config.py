"""
Classifier configuration utilities.

This module provides utility functions for handling classifier configuration
...
def standardize_classifier_config(
    ...
    **kwargs: Any,
    ...
    # If config is a ClassifierConfig
        ...
        # Use existing labels if no new labels provided
        if not final_labels:
            final_labels = config.labels

        # Create a new ClassifierConfig with the updated params
        return config.with_options(labels=final_labels, params=final_params, **kwargs)

...
# Utility functions moved inline for simplicity
...
class PydanticAIMessagePart(BaseModel):
    """Part of a PydanticAI message."""
...
class PydanticAIMessage(BaseModel):
    ...
    kind: Optional[str] = None  # "request" or "response"
    model_name: Optional[str] = None  # Only for responses
    timestamp: Optional[datetime] = None
    parts: Optional[List[PydanticAIMessagePart]] = None


class Thought(BaseModel):
    """Central state container for Sifaka.

    Simplified analytics-focused design that removes PydanticAI data duplication.
    PydanticAI conversation history is the primary memory source - this object
    is for analytics and debugging only.
    """
    ...
    # NOTE: PydanticAI-specific fields removed to eliminate data duplication
    # Use ConversationHistoryAdapter.get_conversation_summary() to access
    # PydanticAI conversation data instead
    ...
    # NOTE: PydanticAI data methods removed to eliminate duplication
    # Use ConversationHistoryAdapter to access PydanticAI conversation data
    # and usage information directly from the agent

...
def create_error_result(
    ...
    log_level_to_use = log_level if log_level is not None else "error"
    log_function = getattr(logger, log_level_to_use, logger.error)
    log_function(
        f"{component_type} error in {component_name}: {str(error)}", exc_info=include_traceback
    )

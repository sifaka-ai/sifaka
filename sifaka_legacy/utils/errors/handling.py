...
def handle_error(
    ...
    # When called from other error handling functions, return the metadata
    # without raising a new exception
...
def handle_component_error(
    ...
    include_traceback: bool = True,
    additional_metadata: Optional[Dict[str, Any]] = None,
) -> Any:  # Use Any to avoid circular import issues
    """
    ...
    from ..errors.results import ErrorResult
...
def create_error_handler(
...
) -> Callable[[Exception, str, str, bool, Optional[Dict[str, Any]]], Any]:
    """
    Create a component-specific error handler.

    This function returns a handler function that can be used to handle errors in a
    component-specific way. The handler creates an ErrorResult with the appropriate
    ...
    Returns:
        Component-specific error handler function
    """
    ...
    def handler(
        ...
        log_function(
            f"{component_type} error in {component_name}: {error_message}",

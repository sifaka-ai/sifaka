...
## Usage Examples
```python
# Import the state management utilities
from sifaka.chain import state
from sifaka.models import OpenAIProvider
from sifaka.chain.validators import ValidationManager

# Create chain state
state_manager = state.create_chain_state()
...
class ChainStateManager(StateManager):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_execution_success": self.get_metadata("last_execution_success", False),
...
def create_chain_state(**kwargs: Any) -> ChainStateManager:
    ...
    This function creates a specialized state manager for chain components.
    It initializes the state manager with chain-specific state attributes
    and provides chain-specific methods for state management.

    Args:
        **kwargs: Keyword arguments for state initialization
    ...
    # Initialize state with provided kwargs
    for key, value in kwargs.items():
        manager.update(key, value)

    # Set default metadata
    manager.set_metadata("component_type", "chain")
    manager.set_metadata("creation_time", time.time())

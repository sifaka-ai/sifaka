...
class BaseComponent(ABC, Generic[T, R]):
    ...
    def handle_empty_input(self, input: T) -> Optional[R]:
        """Handle empty input validation.

        Args:
            input: The input to check for emptiness

        Returns:
            A result object or None if the input is not empty
        """
        from sifaka.utils.text import is_empty_text, handle_empty_text
        from sifaka.core.results import BaseResult, ErrorResult
        from sifaka.utils.errors.results import create_error_result
        from sifaka.utils.errors.base import SifakaError

        # If input is None, return an error result
        if input is None:
            ...
            error_result = create_error_result(
                ...
                metadata={"component_type": self.__class__.__name__},
            )

            # Cast to the expected return type R
            return cast(R, error_result)

        # For string inputs, use text emptiness check
        ...
        # For other empty inputs, return a basic error result
        if not input:
            # Create a generic error result
            error_result = create_error_result(
    ...
    def validate_text_length(
        ...
        # Fast path for empty min/max constraints

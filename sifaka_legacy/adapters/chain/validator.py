...
class ValidatorAdapter(InitializeStateMixin, Validator):
    ...
    def _convert_result(self, result: Union[Any, ErrorResult]) -> ValidationResult:
        ...
        passed = self._get_attribute_safely(result, "passed", False)
        message = self._get_attribute_safely(result, "message", "")
        score = self._get_attribute_safely(result, "score", 0.0)
        issues = self._get_attribute_safely(result, "issues", [])
        suggestions = self._get_attribute_safely(result, "suggestions", [])
        metadata = self._get_attribute_safely(result, "metadata", {})
